...
division a d | (a < d && a >= 0) || (a > -d && a < 0) = (0, a)
             | a < 0 = (fst qrNeg' - 1, snd qrNeg')
             | otherwise = (fst qrPos' + 1, snd qrPos')
    where qrPos' = division (a - d) d
...
solucionParticular a b m | tieneSolucion a b m = mod (b*(fst (division s' g'))) m'
                         | otherwise = undefined
