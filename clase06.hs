...
multiplosDeNPM n [] = []
multiplosDeNPM n (x:xs) 
  | esMultM x n = x:multiplosDeNPM n xs
...
maximoAuxPM [] max = max
maximoAuxPM (x:xs) max 
  | max < x = maximoAuxPM xs x
...
quitarAuxPM n [] i c = []
quitarAuxPM n (x:xs) i c 
  | not (inListPM (x:xs) n) = (x:xs)
...
hayRepetidosAux xs c 
  | c == (longitud xs - 1) = repetidoEnC
