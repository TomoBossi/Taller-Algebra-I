...
distriBolitas n k = agregarCaja k (distriBolitas (n-1) k)
  where 
    agregarCaja :: Int -> [[Int]] -> [[Int]]
    agregarCaja k x = agregarCajaAux k x x
      where 
        agregarCajaAux :: Int -> [[Int]] -> [[Int]] -> [[Int]]
        agregarCajaAux 1 [] _ = []
...
listOrd k n = agregarNoRepe n (listOrd (k-1) n)
  where 
    agregarNoRepe :: Int -> Set (Set Int) -> Set (Set Int)
    agregarNoRepe n x = agregarNoRepeAux n n x x 
      where 
        agregarNoRepeAux :: Int -> Int -> Set (Set Int) -> Set (Set Int) -> Set (Set Int)
        agregarNoRepeAux _ 0 _ _ = []
        agregarNoRepeAux n m [] y = agregarNoRepeAux n (m-1) y y
        agregarNoRepeAux n m (x:xs) y 
          | not (elem m x) = [m:x] ++ agregarNoRepeAux n m xs y
...
subconjuntos k n = filtrarRepe (listOrd k n)
  where 
    filtrarRepe :: [[Int]] -> Set (Set Int)
    filtrarRepe x = filtrarRepeAux x []
      where 
        filtrarRepeAux :: [[Int]] -> Set (Set Int) -> Set (Set Int)
        filtrarRepeAux [] f = f
        filtrarRepeAux (x:xs) f 
          | incluidoSubconj x xs = filtrarRepeAux xs f
          | otherwise = filtrarRepeAux xs (x:f)
          where 
            incluidoSubconj :: Set Int -> [[Int]] -> Bool
            incluidoSubconj _ [] = False
            incluidoSubconj c (x:xs) = iguales c x || incluidoSubconj c xs
              where 
                iguales :: Set Int -> Set Int -> Bool -- clase07
                iguales a b = incluido a b && incluido b a
                  where 
                    incluido :: Set Int -> Set Int -> Bool -- clase07
                    incluido [] b = True
...
sucesion6 = sucesionesBalanceadas 6 -- necesariamente par
  where 
    ...
    sucesionesBalanceadas l = filtrarBalanceadas (var [0,1] l)
      where 
        filtrarBalanceadas :: [[Int]] -> [[Int]]
        filtrarBalanceadas [] = []
        filtrarBalanceadas (x:xs) 
          | c1 == c0 = x:filtrarBalanceadas xs
